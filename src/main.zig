...
// File: src/main.zig
//
// FAA Route Finder — Program entry point.
//
// - Bootstraps the general-purpose allocator.
// - Loads configuration (port from env/.env).
// - Constructs and starts the POSIX TCP listener (server).
// See also:
//   src/server.zig       # socket listener & routing
//   src/config.zig       # configuration loading
...
pub fn main() !void {
    ...
    var server = Server.new(allocator, cfg);
    try server.listen();
}
